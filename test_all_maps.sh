...
echo -e "${YELLOW}Timeout: ${TIMEOUT_MAPS}${NC}"
echo ""
...
if [ ${#TIMEOUT_FILES[@]} -gt 0 ]; then
    echo -e "${YELLOW}=== TIMEOUT MAPS (${#TIMEOUT_FILES[@]}) ===${NC}"
    echo -e "${YELLOW}These maps launched successfully but were terminated after 5s${NC}"
...
if [ $TOTAL_MAPS -gt 0 ]; then
    FUNCTIONAL_MAPS=$((PASSED_MAPS + TIMEOUT_MAPS))
    SUCCESS_RATE=$((FUNCTIONAL_MAPS * 100 / TOTAL_MAPS))
    echo -e "${BLUE}=== ANALYSIS ===${NC}"
    echo -e "Functional maps (passed + timeout): ${FUNCTIONAL_MAPS}/${TOTAL_MAPS} (${SUCCESS_RATE}%)"
